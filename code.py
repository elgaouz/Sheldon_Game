...
MATCHES_FILE = 'output/matches.csv'
...
with open(PLAYER_INFO_FILE, newline='', encoding='utf-8') as file:
    ...
    for row in reader:
        ...
        if name not in players_infos:
            players_infos[name] = []
        players_infos[name].append(row['Sign'])  # Ajouter les signes de chaque round
    ...
    with open(output_file, mode='w', newline='', encoding='utf-8') as file:
        ...
        while len(rounds) > 0:
            ...
            for player1, player2 in rounds:
                ...
                print(f"{player1} ({sign1}) vs {player2} ({sign2}) ➝ 🏆 {winner}")
        ...
        print(f"\n🏆 Le grand gagnant du jeu est : {grand_winner} 🎉")
